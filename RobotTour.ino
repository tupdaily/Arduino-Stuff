...
#define MM_PER_REV              204   // Set to the number of mm per wheel revolution (Hence : Diameter * Pi)
#define ENCODER_COUNTS_90_DEG   240   // Set to the number of encoder pulses to make a 90 degree turn
#define SPEED_MIN               250    // Minimum speed (pulses/second) use at the end of individual moves


LiquidCrystal_I2C display(0x27,16,2);  // set the LCD address to 0x27 for a 20 chars and 4 line display
...
class Command
{
  ...
  public:
    ...
    void clear() {//clears all commands
      start = 0;
      end = 0;
      last_cmd = 0;
      flagFirstScan = 0;
    ...
    void add(int cmd) { //adds command
      add(cmd,0);
    }

    void add(int cmd,int par1) { //adds command
      list[end] = cmd;
      p1[end] = par1;
      end++;
...
void loadCommandQueue() {
  ...
  //    SETTING THE SPEEDS ABOVE THE MOTOR'S MAXIMUM SPEED WILL CAUSE STRANGE RESULTS
      cmdQueue.add(VEHICLE_SET_MOVE_SPEED,340);     // Speed used for forward movements  
      cmdQueue.add(VEHICLE_SET_TURN_SPEED,2000);     // Speed used for left or right turns
      cmdQueue.add(VEHICLE_SET_ACCEL,1000);         // smaller is softer   larger is quicker and less accurate moves

        // Example list of robot movements
        // This block is modified for each tournament //herelol
      cmdQueue.add(VEHICLE_FORWARD,463);
      cmdQueue.add(VEHICLE_FORWARD,308);
      ...
      cmdQueue.add(VEHICLE_FORWARD,463);
      cmdQueue.add(VEHICLE_TURN_RIGHT);
      cmdQueue.add(VEHICLE_FORWARD,385);
...
class MotionLogic
{
  ...
  public:
    ...
    void startMove(int pos, int spd) {//basically only starts the motors and gives initial calculations of accel and velo timings
    ...
    void updateMotion(long usecElapsed, int whichmotor) {//speed controller
      int flagUpdate = 0;
      ...
      if (timerUpdate >= 30000) {
        ...
        Serial.print(speedActual);Serial.print(' ');Serial.print(timerUpdate);Serial.print(' ');Serial.print(usLast);Serial.print(' '); Serial.print(whichmotor);
        Serial.print('\n');
...
void loop() {
  ...
  mtrRight.updateMotion(usecElapsed,1);
  mtrLeft.updateMotion(usecElapsed,0);
  ...
  switch (cmdQueue.current()) {
    ...
    case VEHICLE_FORWARD :
      if (newCmd) {
        ...
        speed = speedFwd;
        mtrLeft.startMove(distance-25,speed);
